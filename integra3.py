# -*- coding: utf-8 -*-
"""
Created on Mon May 26 16:09:45 2014
...
for i in xrange(extra.size):
    ...
    for j in xrange(i+1):
        suma+=f[j,i-j]
        coef[i,0]=suma
    coef[i]=suma*(-1)**(i+1)*facto(i)*exp(-z)/(log(z)-alp)**(i+1)
print
...
for i in xrange(dif):
    ...
    coef[es+i]=suma*(-1)**(es+i+1)*facto(es+i)*exp(-z)/(log(z)-alp)**(es+i+1)
#diagonal inferior  
dif2=extra.size-1
for i in xrange(dif2):
    ...
    for j in xrange(dif2-i):
        ...
        coef[n+1+i,0]=suma
    coef[n+1+i]=suma*(-1)**(n+1+i+1)*facto(n+1+i)*exp(-z)/(log(z)-alp)**(n+1+i+1)
print coef
print
